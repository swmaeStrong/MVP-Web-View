...
export default function TierSystemTooltip() {
  ...
  return (
    ...
    >
      {/* 트리거 버튼 */}
      <button className='flex w-[120px] cursor-help items-center justify-center space-x-2 rounded-lg border border-gray-300 bg-white px-3 py-2 transition-colors duration-200 hover:bg-gray-50'>
      ...
      {/* 툴팁 */}
      {isVisible && (
        <div className='absolute top-full left-0 z-[9999] mt-2 w-[480px] rounded-lg border-2 border-gray-200 bg-white p-5 shadow-xl'>
          {/* 화살표 */}
          ...
          <div className='space-y-2'>
            {tierSystemInfo.map((tier, index) => (
              <div
                key={tier.tier}
                className='flex items-center space-x-3 rounded-lg bg-gray-50/50 p-2'
              >
                <Image
                  src={tier.icon}
                  alt={tier.tier}
